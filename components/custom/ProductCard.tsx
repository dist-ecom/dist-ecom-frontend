...
import { useProductInventory } from "../ProductInventoryContext";
  ...
  return (
    <Card className="overflow-hidden border shadow-sm h-full flex flex-col">
      ...
      <CardFooter className="flex flex-col items-start p-4 space-y-2 flex-1">
        <h3 className="font-medium line-clamp-2 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
          {product.name}
        </h3>
        ...
        {isOpen && (
          <div className="mt-2 space-y-2">
            ...
            {productData && (
              <p className={`text-xs font-medium ${productData.stock <= productData.lowStockThreshold ? 'text-red-500' : 'text-gray-700'}`}>
                Stock: {productData.stock}
              </p>
            )}

            {productData && productData.stock <= productData.lowStockThreshold && (
